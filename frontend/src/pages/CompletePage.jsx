    ...
    .sort((a, b) => {
      switch (sortBy) {
        ...
        case 'priority': {
          ...
          return (order[b.priority?.toLowerCase()]) - (order[a.priority?.toLowerCase()] || 0);
        }
        default:
          return 0;
      }
        ...
        <div className={CT_CLASSES.sortContainer}>
          <div className={CT_CLASSES.sortBox}>
            <div className={CT_CLASSES.filterLabel}>
              ...
              <span className='text-xs md:text-sm '>Sortb by:</span>
            </div>
            {/* MOBILE DROPDOWN */}
<select
  value={sortBy}
  ...
  {sortedCompletedTasks.length === 0 ? (
    ...
    sortedCompletedTasks.map((task) => (
      <TaskItem
        ...
        className="opacity-98 hover:opacity-100 transition-opacity text-sm md:text-base"
      />
    ))
  )}
</div>
